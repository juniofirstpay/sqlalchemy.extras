...
async_session_context: ContextVar[AsyncSession] = ContextVar("async_session_context", default=None)
async_sessionmaker_context: ContextVar[async_sessionmaker] = ContextVar("async_sessionmaker_context", default=None)
    ...
    if len(args) > 0:
        ...
        if len(args) == 1:
            def session_wrap(async_callable):
                return _session_wrap(session_factory, async_callable, nested=nested_transaction_flag)

            return session_wrap
        
        return session_wrap(session_factory, async_callable, nested=nested_transaction_flag)
